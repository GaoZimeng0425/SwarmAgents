...
export type ToolHeaderProps = {
  title?: string
  className?: string
  /** One-line result preview shown next to the name while the card is collapsed. */
  preview?: string
} & (
...
export const ToolHeader = ({ className, title, type, state, toolName, preview, ...props }: ToolHeaderProps) => {
  ...
  return (
    <CollapsibleTrigger className={cn('flex w-full items-center gap-2 px-3 py-2 text-left', className)} {...props}>
      <span className="shrink-0">{statusIcons[state]}</span>
      <span className="shrink-0 font-medium font-mono text-[13px]">{title ?? derivedName}</span>
      {preview && (
        <span className="min-w-0 flex-1 truncate text-muted-foreground/70 text-xs group-data-[state=open]:hidden">
          {preview}
        </span>
      )}
