...
// react-markdown strips unknown URL schemes by default; let our private one through.
const messageUrlTransform: UrlTransform = (url, key, node) =>
  url.startsWith(FILE_LINK_SCHEME) ? url : defaultUrlTransform(url, key, node)

// A file-path link opens the file with the OS instead of navigating the renderer.
const messageComponents: Components = {
  a: ({ href, children, node: _node, ...rest }) => {
    if (typeof href === 'string' && href.startsWith(FILE_LINK_SCHEME)) {
