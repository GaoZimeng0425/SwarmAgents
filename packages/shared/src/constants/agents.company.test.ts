...
// Doers (engineer, reviewer) use their full tool access directly. toolScope no
// longer encodes this distinction (collapsed to the authoring gate), so this
// only checks each role is a valid, non-empty definition.
const COMPANY_ROLES = ['ceo', 'engineering-lead', 'engineer', 'reviewer'] as const

describe('company role agent definitions', () => {
  it('ships ceo/engineering-lead/engineer/reviewer as valid definitions', () => {
    for (const id of COMPANY_ROLES) {
