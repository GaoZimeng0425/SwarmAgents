...
describe('toRendererEvent', () => {
  it('re-attaches the SSE event name as the UIEvent kind', () => {
    const e = toRendererEvent('run.created', { runId: 't1', prompt: '你好', ts: 1 })
    expect(e).toEqual({ kind: 'run.created', runId: 't1', prompt: '你好', ts: 1 })
  })
  ...
  it('forwarded events flow through applyEvent without becoming a stub', () => {
    const created = toRendererEvent('run.created', { runId: 't1', prompt: '你好', ts: 1, seq: 1 })
