...
describe('toRendererEvent', () => {
  it('re-attaches the SSE event name as the UIEvent kind', () => {
    const e = toRendererEvent('message.created', { messageId: 't1', prompt: '你好', ts: 1 })
    expect(e).toEqual({ kind: 'message.created', messageId: 't1', prompt: '你好', ts: 1 })
  })

  // Regression for the "(unknown run)" / stuck-running bug: a forwarded
  // run.created must create a real record with the user's prompt, not a stub.
  it('forwarded events flow through applyEvent without becoming a stub', () => {
    const created = toRendererEvent('message.created', { messageId: 't1', prompt: '你好', ts: 1, seq: 1 })
    const afterCreate = applyEvent([], created)
    expect(afterCreate[0]).toMatchObject({ id: 't1', prompt: '你好', status: 'pending' })

    const complete = toRendererEvent('message.complete', { messageId: 't1', summary: 'done', ts: 2, seq: 2 })
