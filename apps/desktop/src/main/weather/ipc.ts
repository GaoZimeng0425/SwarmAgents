...
function resolveForecast(
  ...
  return service
    ...
    .then((forecast) => {
      if (broadcast) {
        sendToAllWindows('weather:forecastChanged', forecast)
      }
...
export function wireWeatherIpc(args: { service: Service }): {
  ...
  ipc.handle('weather:getConfig', () => service.getConfig())

  ipc.handle('weather:setConfig', (_e, c: unknown) => {
  ...
  ipc.handle('weather:getForecast', (_e, lng: unknown, lat: unknown) => {
