// src/main/calendar/auth.ts
//
// Calendar OAuth via a loopback HTTP server (Google's installed-app flow).
// Opens the consent URL in the system browser; the loopback server captures
// the ?code= redirect, exchanges it for tokens, and persists them (encrypted).
// Token refresh runs on-demand when getAccessToken sees an expired token.
// Mirrors gmail/auth.ts; scope is calendar.readonly.
import { createServer, type Server } from 'node:http'
import type { AddressInfo } from 'node:net'
import { createLogger } from '@shared/logger'
import type { CalendarClientCreds, CalendarConfigOnDisk, CalendarTokens } from '@swarm/protocol'
import { shell } from 'electron'

import { oauthErrorHtml, oauthSuccessHtml } from '../oauth-redirect'
import type { Store } from './store'

const log = createLogger({ process: 'main' }).child({ component: 'calendar-auth' })
...
export type AuthDeps = {
  store: Store
  /** Fetch the account profile once tokens land; returns { emailAddress }. */
...
export function createAuth(deps: AuthDeps): Auth {
  ...
  const persistTokens = async (next: CalendarTokens, email?: string): Promise<void> => {
    const cur = await store.load()
    const state: CalendarConfigOnDisk = {
  ...
  const getAccessToken: Auth['getAccessToken'] = async () => {
    ...
    if (!cfg.tokens) throw new Error('Calendar not linked')
    const skew = 60_000
    if (cfg.tokens.expiresAt - skew > Date.now()) return cfg.tokens.accessToken
    await refreshAccessToken()
    const after = await store.load()
    if (!after.tokens) throw new Error('Calendar not linked after refresh')
    ...
    new Promise<void>((resolve, reject) => {
      store.load().then((cfg) => {
        ...
        const deadline = setTimeout(() => {
          log.warn({ msg: 'calendar login timed out' })
        ...
        server = createServer(async (req, res) => {
          ...
          try {
            ...
            log.info({ msg: 'calendar login captured', email: profile.emailAddress })
            res.writeHead(200, { 'content-type': 'text/html; charset=utf-8' })
            res.end(oauthSuccessHtml(profile.emailAddress))
            finish(() => resolve())
          } catch (err) {
            const errMsg = err instanceof Error ? err.message : String(err)
            log.error({ msg: 'calendar login exchange failed', err: errMsg })
        ...
        server.listen(0, '127.0.0.1', () => {
          ...
          log.info({ msg: 'calendar consent opening', redirectUri: redirectUriHeld })
  ...
  const logout: Auth['logout'] = async () => {
    ...
    log.info({ msg: 'calendar logged out' })
  }

  return { login, logout, getAccessToken, refreshAccessToken }
}

// Exported for tests / index.ts to build the consent scope assertion.
export const CALENDAR_SCOPE = SCOPE
