...
export function GanttView(props: {
  ...
  // Measure container width so the day grid fills the available space.
  useLayoutEffect(() => {
  ...
  // Stretch dayWidth to fill the container (after subtracting the frozen name
  // column), but never narrower than MIN_DAY_WIDTH so cells stay legible.
  const availableW = Math.max(0, containerW - NAME_W)
  const dayWidth = availableW > 0 ? Math.max(MIN_DAY_WIDTH, Math.floor(availableW / days)) : MIN_DAY_WIDTH
  ...
  // The name column + day columns share one scroll container so the header
  // strip and body grid stay horizontally aligned. Header is sticky at top;
  // name column is sticky at left.
  return (
    <div
      ...
      onPointerUp={endDrag}
    >
      <div className="cmdscroll min-h-0 flex-1 overflow-auto" ref={containerRef}>
        <div style={{ width: NAME_W + contentW }}>
          {/* Sticky header: name spacer + date strip */}
          <div className="sticky top-0 z-20 flex bg-background" style={{ height: HEADER_H }}>
            <div className="sticky left-0 z-10 shrink-0 bg-background" style={{ width: NAME_W }} />
            <div className="flex">
          ...
          <div className="mb-1 h-px w-full" style={{ backgroundColor: 'var(--color-border)' }} />
          ...
          {scheduled.map((row) => {
            ...
            return (
              <div className="relative flex items-center" key={row.id} style={{ height: ROW_H, marginBottom: ROW_GAP }}>
                {/* Sticky name cell */}
                <button
                  className="sticky left-0 z-10 flex shrink-0 items-center truncate bg-background px-2 text-left text-[12px] text-foreground"
                ...
                {/* Timeline area */}
                <div className="relative" style={{ width: contentW, height: ROW_H }}>
                  {/* Grid lines */}
                  {Array.from({ length: days }, (_, i) => {
                    const date = new Date(timelineStart)
                    date.setDate(date.getDate() + i)
                    ...
                    return (
                      <div
                        className="absolute top-0 h-full border-border/40 border-l"
                        key={date.toDateString()}
                        style={{
                          width: dayWidth,
                          left: i * dayWidth,
      ...
      {unscheduled.length > 0 && (
        <div className="cmdscroll mt-1 flex max-h-32 flex-none flex-col gap-1.5 overflow-y-auto pt-2">
