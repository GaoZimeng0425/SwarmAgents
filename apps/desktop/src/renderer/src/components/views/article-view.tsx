          ...
          <div className="min-h-0 flex-1" ref={measureRef}>
            <ScrollArea className="h-full">
              <div className="grid gap-3" style={{ gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))` }}>
                {articles.map((a) => (
                  <ArticleCard article={a} key={a.id} onClick={setSelected} selected={selected?.id === a.id} />
                ))}
              </div>
            </ScrollArea>
          </div>
        )}
        <ArticleDetailPanel article={selected} onChanged={() => setSelected(null)} onClose={() => setSelected(null)} />
      </div>
    </div>
  )
}
