...
describe('classifyComposerTurns', () => {
  it('treats a lone pending turn as active, not queued (the created→dispatched window)', () => {
    ...
    // staging card.
    const { activeTask, queuedTasks } = classifyComposerTurns([rec({ id: 't1', status: 'pending', createdAt: 100 })])
    expect(activeTask?.id).toBe('t1')
    expect(queuedTasks).toEqual([])
  })

  it('queues pending turns behind a running turn', () => {
    const running = rec({ id: 'r', status: 'running', createdAt: 100 })
    const queued = rec({ id: 'q', status: 'pending', createdAt: 200 })
  ...
  it('promotes the earliest pending turn and queues the rest when nothing is running', () => {
    const a = rec({ id: 'a', status: 'pending', createdAt: 300 })
    const b = rec({ id: 'b', status: 'pending', createdAt: 100 })
    const c = rec({ id: 'c', status: 'pending', createdAt: 200 })
  ...
  it('treats an awaiting_user turn as the active turn', () => {
    const awaiting = rec({ id: 'w', status: 'awaiting_user', createdAt: 100 })
    const pending = rec({ id: 'p', status: 'pending', createdAt: 200 })
  ...
  it('ignores sub-agent children — only top-level turns count', () => {
    const child = rec({ id: 'c', status: 'pending', createdAt: 100, parentMessageId: 'top' })
  ...
  it('has no active turn when all top-level turns are terminal', () => {
    const done = rec({ id: 'd', status: 'completed', createdAt: 100 })
  ...
  it('keeps the active (running and starting) turns in the transcript', () => {
    const running = rec({ id: 'r', status: 'running', createdAt: 100 })
    const done = rec({ id: 'd', status: 'completed', createdAt: 50 })
  ...
  it('keeps sub-agent children in the transcript', () => {
    const running = rec({ id: 'r', status: 'running', createdAt: 100 })
    const child = rec({ id: 'c', status: 'pending', createdAt: 150, parentMessageId: 'r' })
    const queued = rec({ id: 'q', status: 'pending', createdAt: 200 })
  ...
  it('renders pending turns as transcript (not queued cards) for a non-active session', () => {
    ...
    const a = rec({ id: 'a', status: 'pending', createdAt: 100 })
    const b = rec({ id: 'b', status: 'pending', createdAt: 200 })
