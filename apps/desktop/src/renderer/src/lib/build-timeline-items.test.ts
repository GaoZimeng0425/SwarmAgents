...
const task = (id: string, events: TaskRecord['events'], startedAt = 1): TaskRecord =>
  ({ id, sessionId: 's', goal: `goal-${id}`, status: 'running', workerId: null, summary: null, startedAt, attachments: [], events }) as unknown as TaskRecord
...
describe('buildTimelineItems', () => {
  ...
  it('interleaves two tasks by seq regardless of array order', () => {
    const a = task('ta', [{ kind: 'task.created', sessionId: 's', taskId: 'ta', goal: 'a', ts: 1, seq: 5 } as TaskRecord['events'][number]])
    const b = task('tb', [{ kind: 'task.created', sessionId: 's', taskId: 'tb', goal: 'b', ts: 1, seq: 1 } as TaskRecord['events'][number]])
