  ...
  ({ className, variant = 'filled', ...props }, ref) => {
    return (
      <UITabs
        ...
        // @ts-expect-error - pass variants to context
        context={{ variant }}
      />
    )
  }
...
const TabsList = React.forwardRef<React.ComponentRef<typeof UITabs.List>, ITabsListProps>(
  ({ className, children, ...props }, ref) => {
    ...
    useEffect(() => {
      ...
      const triggers = childArray.filter((child: any) => child?.type?.displayName !== 'TabsIndicator')
    ...
    if (orientation === 'horizontal') {
      return (
        <View className={tabsListStyle({ orientation, class: className })} ref={listRef}>
          {indicator}
...
const TabsContent = React.forwardRef<React.ComponentRef<typeof UITabs.Content>, ITabsContentProps>(
  ({ className, ...props }, ref) => {
    return <UITabs.Content ref={ref} {...props} className={tabsContentStyle({ class: className })} />
  }
)

const TabsContentWrapper = React.forwardRef<React.ComponentRef<typeof UITabs.ContentWrapper>, ITabsContentWrapperProps>(
...
const TabsIndicator = React.forwardRef<React.ComponentRef<typeof UITabs.Indicator>, ITabsIndicatorProps>(
  ({ className, ...props }, ref) => {
    ...
    // @ts-expect-error - Get animated scroll offset from context
...
export { Tabs, TabsContent, TabsContentWrapper, TabsIndicator, TabsList, TabsTrigger, TabsTriggerIcon, TabsTriggerText }
