...
export const PromptInputProvider = ({ children }: { children: ReactNode }) => {
  ...
  return <AttachmentsContext.Provider value={attachmentsValue}>{children}</AttachmentsContext.Provider>
}

// ====================== REST OF YOUR COMPONENTS (unchanged) ======================

export const PromptInput = ({ children, onSubmit }: { children: ReactNode; onSubmit?: any }) => {
  ...
  return (
    <PromptContext.Provider value={{ text, setText, handleSubmit, isDisabled }}>
      <Animated.View style={inputAnimatedStyle}>
        <View className="absolute bottom-4 w-full rounded-3xl border-border bg-muted px-3 py-2">{children}</View>
      </Animated.View>
...
export const PromptInputBody = ({ children }: { children: ReactNode }) => (
  <View className="flex-row items-center gap-2 pt-2">{children}</View>
)
...
export const PromptInputTextarea = () => {
  ...
  return (
    <TextInput
      className="flex-1 items-center justify-center rounded-3xl px-4 text-foreground text-xl placeholder:text-muted-foreground focus:border-transparent focus:outline-none focus:ring-0"
      multiline
...
export const PromptInputFooter = ({ children }: { children: ReactNode }) => (
  <View className="mt-6 flex-row items-center justify-between">{children}</View>
)
...
export const PromptInputButton = ({ children }: { children: ReactNode }) => (
  <TouchableOpacity className="rounded-full bg-muted px-3 py-1">
    <Text>{children}</Text>
  </TouchableOpacity>
)
...
export const PromptInputSubmit = () => {
  ...
  return (
    <TouchableOpacity
      className={`mx-3 h-10 w-10 items-center justify-center rounded-full bg-primary ${isDisabled ? 'opacity-50' : ''}`}
      disabled={isDisabled}
...
export const PromptInputActionMenuTrigger = ({ children, ...props }: any) => (
  <TouchableOpacity {...props}>
    <View className="h-10 w-10 items-center justify-center rounded-full bg-primary/10">
      {children ?? <Text className="text-primary text-xl">+</Text>}
    </View>
  </TouchableOpacity>
)

export const PromptInputActionMenuContent = ({ trigger }: { trigger: (props: any) => React.ReactNode }) => {
  const attachments = usePromptInputAttachments()
  ...
  return (
    <Menu offset={5} placement="top" trigger={trigger}>
